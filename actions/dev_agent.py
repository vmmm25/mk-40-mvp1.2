...
def _get_client_and_model(model_name: str):
    """Return (client, model_name) using google.genai (new API)."""
    from google import genai
    client = genai.Client(api_key=_get_api_key())
    return client, model_name


def _genai_generate(client, model_name: str, prompt: str):
    """Helper: call generate_content and return the response (has .text)."""
...
def _plan_project(description: str, language: str) -> dict:
    client, model_name = _get_client_and_model(MODEL_PLANNER)
...
JSON:"""
    ...
    try:
        response = _genai_generate(client, model_name, prompt)
...
def _write_file(
    ...
    client, model_name = _get_client_and_model(MODEL_WRITER)
