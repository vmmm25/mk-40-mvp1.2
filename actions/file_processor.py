...
class _GeminiModel:
    """Wrapper that bridges google.genai.Client into the old GenerativeModel interface.

    All call sites remain unchanged — .generate_content() works the same way.
    Handles text, image (PIL), and audio blob contents transparently.
    """
    ...
    def generate_content(self, contents):
        ...
        # Convert legacy dict blobs (mime_type + data) → types.Part objects
        ...
        return self._client.models.generate_content(
            model=self._model, contents=processed
        )
