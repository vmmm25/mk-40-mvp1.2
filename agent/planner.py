"""MARK XL — Provider-agnostic task planner.

Breaks user goals into executable steps using any configured AI provider.
"""
...
import json
import re
from .llm_helper import ask_llm_sync as ask_llm
...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    try:
        text = ask_llm(user_input, PLANNER_PROMPT)
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        text = ask_llm(prompt, PLANNER_PROMPT)
        text = re.sub(r"```(?:json)?", "", text).strip().rstrip("`").strip()
        plan = json.loads(text)
